...
ProxyMap::ProxyMap(const std::string& bpf_root, BpfMetadataStats& stats) : stats_(stats) {
  // Open the bpf maps from Cilium specific paths
  // TODO: make these paths configurable!
  if (!proxy4map_.open(bpf_root + "/tc/globals/cilium_proxy4")) {
    stats_.bpf_open_error_.inc();
  }
  if (!proxy4map_.open(bpf_root + "/tc/globals/cilium_proxy6")) {
    stats_.bpf_open_error_.inc();
  }
}
...
bool ProxyMap::getBpfMetadata(Network::AcceptSocket& socket) {
  ...
  if (local_address->type() == Network::Address::Type::Ip &&
    ...
    if (ip->version() == Network::Address::IpVersion::v4 && rip->version() == Network::Address::IpVersion::v4) {
      ...
      key.saddr = htonl(ip->ipv4()->address());
	...
	socket.setSocketMark(value.identity);
	return true;
      }
      ...
      memcpy(&key.saddr, &ip->ipv6()->address()[0], 16);
      key.dport = htons(ip->port());
      key.sport = htons(rip->port());
      key.nexthdr = 6;
